...
class ROCAnalysisScreen(QWidget, WorkflowNodeScreenSupport):
    """ROC Analysis — Plot Receiver Operating Characteristic curve."""
    ...
    def _compute(self) -> None:
        ...
        if not is_clf:
            self._info_label.setText("ROC Analysis only supports classifiers.")
            self._clear_figure()
            return
        ...
        try:
            ...
            self._info_label.setText(f"Computed ROC for {len(y_actual)} instances.")
            self._redraw()
    ...
    def _save_image(self) -> None:
        if self._figure is None or self._y_true is None:
            return
        path, _ = QFileDialog.getSaveFileName(self, "Save ROC Plot", "roc_curve.png", "PNG Image (*.png);;All Files (*)")
        if path:
    ...
    def _redraw(self) -> None:
        ...
        try:
            ...
            fpr, tpr, _ = skl_metrics.roc_curve(y_true_binary, y_score)
            auc = skl_metrics.roc_auc_score(y_true_binary, y_score)

            ax.plot(fpr, tpr, color="#2196F3", lw=2, label=f"AUC = {auc:.3f}")
            ax.plot([0, 1], [0, 1], color="#999999", lw=1, linestyle="--")
            ax.set_xlim([0.0, 1.0])
            ax.set_ylim([0.0, 1.05])
            ax.set_xlabel("False Positive Rate")
            ax.set_ylabel("True Positive Rate")
            ax.set_title(f"ROC Curve (Class: {self._class_values[target_idx]})")
