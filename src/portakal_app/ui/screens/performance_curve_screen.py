...
class PerformanceCurveScreen(QWidget, WorkflowNodeScreenSupport):
    """Performance Curve — Plot Precision-Recall curve."""
    ...
    def _compute(self) -> None:
        ...
        if not is_clf:
            self._info_label.setText("Performance Curve only supports classifiers.")
            self._clear_figure()
            return
        ...
        try:
            ...
            self._info_label.setText(f"Computed Performance Curve for {len(y_actual)} instances.")
            self._redraw()
    ...
    def _save_image(self) -> None:
        if self._figure is None or self._y_true is None:
            return
        path, _ = QFileDialog.getSaveFileName(self, "Save Precision-Recall Plot", "pr_curve.png", "PNG Image (*.png);;All Files (*)")
        if path:
    ...
    def _redraw(self) -> None:
        ...
        try:
            ...
            precision, recall, _ = skl_metrics.precision_recall_curve(y_true_binary, y_score)
            ap = skl_metrics.average_precision_score(y_true_binary, y_score)

            ax.plot(recall, precision, color="#4CAF50", lw=2, label=f"AP = {ap:.3f}")
            ax.set_xlim([0.0, 1.0])
            ax.set_ylim([0.0, 1.05])
            ax.set_xlabel("Recall")
            ax.set_ylabel("Precision")
            ax.set_title(f"Precision-Recall Curve (Class: {self._class_values[target_idx]})")
